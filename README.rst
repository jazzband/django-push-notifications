...
- ``AUTO_UPDATE_MATCHING_REG_ID``: Transform create of an existing Device (based on registration id) into a update. See below `Auto update of device with same registration ID`_ for more details.
...
Auto update of device with same registration ID
-----------------------------------------------
...
When option ``AUTO_UPDATE_MATCHING_REG_ID`` is set to True, then any creation of
device with an already existing registration ID will be transformed into an update.

The ``AUTO_UPDATE_MATCHING_REG_ID`` only works with DRF.
