...
def _apns_select_certificate(app_id):
        certfile =  (SETTINGS["APNS_CERTIFICATES"].get(app_id)
                     or SETTINGS["APNS_CERTIFICATES"].get("default"))
        if not certfile:
		raise ImproperlyConfigured(
                        'You need to set PUSH_NOTIFICATIONS_SETTINGS["APNS_CERTIFICATES"] to send messages through APNS.'
		)
        return certfile
...
def _apns_send(token, alert, badge=None, sound=None, category=None, content_available=False,
               ...
               expiration=None, priority=10, socket=None, app_id=None):
	data = {}
	aps_data = {}
...
# TODO: apns_send_bulk_message needs API change to handle multi-app flow
#
