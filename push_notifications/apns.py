...
def _apns_send(
    ...
    if priority:
        try:
            notification_kwargs["priority"] = apns2_client.NotificationPriority(
                str(priority)
            )
        except ValueError:
    ...
    if batch:
        data = [
            apns2_client.Notification(
                token=rid, payload=_apns_prepare(rid, alert, **kwargs)
            )
...
def apns_send_message(
    registration_id, alert, application_id=None, creds=None, **kwargs
):
    """
    ...
    interruption_level = kwargs.pop("interruption_level", None)

    try:
        if interruption_level:
            apns_send_with_with_interruption_level(
                registration_id,
                alert,
                creds=creds,
...
def apns_send_with_with_interruption_level(
    ...
    **kwargs,
    ...
    return _apns_send(
        registration_id, alert, batch=batch, creds=creds, **notification_kwargs
    )


def _apns_prepare_with_interruption_level(
    token, alert, application_id, interruption_level=InterruptionLevel.ACTIVE, **kwargs
):
    ...
    match interruption_level:
        case InterruptionLevel.CRITICAL_ALERT:
            ...
            notification_kwargs["priority"] = (
                apns2_client.NotificationPriority.Immediate
            )
            notification_kwargs["notification_type"] = (
                apns2_client.NotificationType.Alert
            )
        ...
        case InterruptionLevel.PASSIVE:
            ...
            notification_kwargs["notification_type"] = (
                apns2_client.NotificationType.Background
            )
    ...
    return _apns_prepare(
        token, alert, application_id=application_id, **notification_kwargs
    )
