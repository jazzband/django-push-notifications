...
class GCMDeviceQuerySet(models.query.QuerySet):
    def send_message(self, message, **kwargs):
        if self:
            ...
            reg_ids = self.filter(active=True).values_list(
                'registration_id', flat=True)
            return gcm_send_bulk_message(
                registration_ids=reg_ids, data=data, **kwargs)
...
class GCMDevice(Device):
    ...
    device_id = HexIntegerField(
        _('Device ID'), blank=True, null=True, db_index=True,
    ...
    def send_message(self, message, **kwargs):
        ...
        return gcm_send_message(
            registration_id=self.registration_id, data=data, **kwargs)
    ...
    def send_message(self, message, **kwargs):
        if self:
            ...
            return apns_send_bulk_message(
                registration_ids=reg_ids, alert=message, **kwargs)
...
class APNSDevice(Device):
    ...
    registration_id = models.CharField(
        _('Registration ID'), max_length=64, unique=True)
    ...
    def send_message(self, message, **kwargs):
        ...
        return apns_send_message(
            registration_id=self.registration_id, alert=message, **kwargs)
